...
def conv_block(x, strides):
    net = {}
    net['conv'] = Conv2D(filters=64, kernel_size=(1, 1), strides=strides)(x)
    return BatchNormalization()(net['conv'])
...
def encoder(x):
    ...
    net['main_conv_1'] = Conv2D(filters=64, kernel_size=(3, 3), padding='same', strides=(1, 1))(x)
    net['batch_norm_1'] = BatchNormalization()(net['main_conv_1'])
    net['activation_1'] = Activation(activation='relu')(net['batch_norm_1'])

    main_path = Conv2D(filters=64, kernel_size=(3, 3), padding='same', strides=(1, 1))(net['activation_1'])

    shortcut = {}
    shortcut['input'] = x
    # shortcut['conv'] = Conv2D(filters=64, kernel_size=(1, 1), strides=(1, 1))(shortcut['input'])
    # shortcut['batch_norm'] = BatchNormalization()(shortcut['conv'])
    shortcut['batch_norm_1'] = conv_block(shortcut['input'], (1, 1))
    main_path = concatenate([shortcut['batch_norm_1'], main_path])
    # first branching to decoder
    to_decoder.append(main_path)

    main_path = res_block(main_path, [128, 128], [(2, 2), (1, 1)])

    shortcut['batch_norm_2'] = conv_block(shortcut['input'], (2, 2))
    main_path = concatenate([shortcut['batch_norm_2'], main_path])

    to_decoder.append(main_path)

    main_path = res_block(main_path, [256, 256], [(2, 2), (1, 1)])
    shortcut['batch_norm_3'] = conv_block(shortcut['input'], (4, 4))
    main_path = concatenate([shortcut['batch_norm_3'], main_path])
